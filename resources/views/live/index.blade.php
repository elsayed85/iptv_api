    ...
    <ul id="primary-menu" class="menu">
        @foreach ($categories as $category)
            <li id="menu-item-33" class="menu-item menu-item-type-taxonomy menu-item-object-category menu-item-33"><a
                    href="{{ route('category.show', ['type' => 'lives', 'category' => $category['category_id']]) }}">
        ...
        @foreach ($lives as $live)
            <li class="king-post-item">
                <article id="post-75"
                    ...
                    <a href="{{ route('live.load', $live['stream_id']) }}" class="entry-image-link king-share-link">
                        <div class="entry-image" style="height:480px;">
                            <div class="king-box-bg" data-king-img-src="{{ $live['stream_icon'] }}">
                            </div>
                        </div>
                        @if ($category = $live['category'])
                            <div class="editors-badge">{{ $category }}</div>
                        @endif
                    </a>
                    <div class="post-featured-trending">
                        <div class="king-postext king-ext-75">
                            <a href="{{ route('live.load', $live['stream_id']) }}" class="king-share-link king-readlater"
                    ...
                    <header class="entry-header">
                        <h2 class="entry-title">
                            <a href="{{ route('live.show', $live['stream_id']) }}" rel="bookmark">
                                {{ $live['name'] }}
                            </a>
                        </h2>
                    </header><!-- .entry-header -->
                    <div class="article-meta-04">
                        <div class="entry-meta-left">
                            <span class="content-04-avatar">
                                <a class="content-04-user" href="{{ route('live.show', $live['stream_id']) }}">
                                    {{ $live['name'] }}
                                </a>
                            </span>
                        </div>
                        <div class="entry-meta-right">
                            <div class="entry-meta">
                                ...
                                <span class="post-views"><i class="fa fa-clock" aria-hidden="true"></i>
                                    {{ Carbon\Carbon::createFromTimestamp($live['added'])->diffForHumans() }}
                                </span>
                            </div><!-- .entry-meta -->
